...
class Account():
    ...
    def buy(self, day, contract, price):  # 做多
        ...
        if params.amount == None:
            params.initial_amount = params.mul * price  # 初始成本，用于计算利率
            params.amount = params.initial_amount

        buy_fee = params.fee * price * params.mul * self.contract_num
        self.fee = buy_fee
    ...
    def sell_short(self, day, contract, price):  # 做空
        global df_make_order_record
        global params
        ...
        row = pd.DataFrame([[day, contract, self.contract_num, 'short', price, buy_fee, None, params.amount]],
                           index=[day],
                           columns=['Date', 'FutureID', 'ContractNum', 'Action', 'Price', 'Fee', 'Profit',
                                    'Amount'])
        df_make_order_record = pd.concat([df_make_order_record, row], axis=0)
    ...
    def sell(self, day, price):  # 平多
        ...
        if day == self.open_day:
            fee_pct = params.fee * 15
        else:
            fee_pct = params.fee

        sell_fee = fee_pct * params.mul * price * self.contract_num
        # sell_and_get = self.contract_num * price * params.mul - sell_fee

        amount_new = params.amount + (price-self.open_price) * params.mul - sell_fee
        profit = (price-self.open_price) * params.mul - sell_fee - self.fee
        params.amount = amount_new
        ...
        row = pd.DataFrame(
            [[day, sell_contract, self.contract_num, 'close_pos', price, sell_fee, profit, params.amount]],
            index=[day],
            columns=['Date', 'FutureID', 'ContractNum', 'Action', 'Price', 'Fee', 'Profit', 'Amount'])
        df_make_order_record = pd.concat([df_make_order_record, row], axis=0)
    ...
    def buy_to_cover(self, day, price):  # 平空
        global df_make_order_record
        global df_win
        if day == self.open_day:
            fee_pct = params.fee * 15
        ...
        sell_fee = fee_pct * params.mul * price * self.contract_num
        amount_new = params.amount + (self.open_price - price) * params.mul - sell_fee
        profit = (self.open_price-price) * params.mul - sell_fee - self.fee
        params.amount = amount_new
    ...
    def lock(self, day, acct2, price):
        if acct2.position == 'long':
            self.sell_short(day, acct2.contract, price)
        elif acct2.position == 'short':
            self.buy(day, acct2.contract, price)
        else:
...
class Params():  # 参数
    ...
    # 更新回测期间的交易日期
    def update_dates(self, df):
        self.tradingdates = df.Date.unique()
        # self.settledates = df.SettleDate.unique()
...
def load_data(path, open_data_path, daily_data_path, startdate):
    ...
    params.update_dates(df)
    params.update_settledates(df_olhc)

    tradingdates = params.tradingdates.tolist()
...
def update_portfolio_before_close(day, df_olhc):
    ...
    for acc in [acct, acct_lc]:
        if acc.contract == None:
            pass
        else:
            trade_data = df_olhc.loc[(df_olhc['Date'] == day) & (df_olhc['StockID'] == acc.contract)].iloc[0]

            if acc.position == 'short':
                profit = acc.contract_num * (acc.open_price - trade_data['close'])  # 做空价格-收盘价
                params.netvalue = params.amount + profit

            elif acc.position == 'long':
                profit = acc.contract_num * (trade_data['close'] - acc.open_price)  # 做空价格-收盘价
                params.netvalue = params.amount + profit

            if acc.position == 'long' and trade_data['close'] > trade_data['open']:
                params.daily_win = True
            elif acc.position == 'short' and trade_data['close'] < trade_data['open']:
                params.daily_win = True
            else:
    ...
    # print(str(day) + ':' + str(params.contract))
...
def summary(df_result, df_win):
    ...
    # 交易胜率
    trades_win_ratio = sum(df_win['Win'] == True) / len(df_win)
...
if __name__ == "__main__":
    ...
    startdate = 20180801
    signal_threshs = [0.01]  # , 0.005, 0.01, 0.03, 0.05]

    for code in future_code_list:
        for signal_thresh in signal_threshs:
            path = 'data/' + code + "_201704_202008.csv"
            open_data_path ='data/'+ code + "_OpenData_201704_202008.csv"
            daily_data_path ='data/' + code + "DailyTradeData.csv"

            params = Params(signal_thresh)
            acct = Account()
            acct_lc = Account()

            df_result = pd.DataFrame(columns=['FutureID', 'ContractNum', 'Position', 'Value', 'Win'])
            df_win = pd.DataFrame(columns=['Win'])
            df_make_order_record = pd.DataFrame(columns=['FutureID', 'Action', 'Price', 'Fee', 'Profit', 'Amount'])

            # handle_data
            df_PV, df_olhc, df_signal = handle_data(path, open_data_path, daily_data_path, startdate)

            for day in df_PV.index[1:]:
                run_daily(day, df_signal, df_PV, df_olhc)

            # summary
            df_summary = summary(df_result, df_win)

            pre_path = 'result/' + code +'_original_SL_'+ str(params.thresh) + '_' + str(
                startdate) + '/'
